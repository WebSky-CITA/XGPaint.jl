...
function Battaglia16KinematicSZProfile(; Omega_c::T=0.2589, Omega_b::T=0.0486, h::T=0.6774, x::T=2.6408, vel::T=0.01, mu::T=1.0) where {T <: Real}
    ...
    vel = vel
    mu = mu
    return Battaglia16KinematicSZProfile(f_b, cosmo, X, vel, mu)
end
...
function SZpack_ksz(𝕡, M_200, z, r; τ=0.01, showT=true)
    """
    ...
    # Term 1
    dI_1 = uconvert(u"kg*s^-2",(szpack_interp_ksz(t, 𝕡.vel, 𝕡.mu, nu)*u"MJy/sr" - szpack_interp_T0(𝕡.vel, 𝕡.mu, nu)*u"MJy/sr")/τ)
    y = XGPaint.compton_y_ksz(𝕡, M_200, z, r)
    I_1 = uconvert(u"kg*s^-2",y * (dI_1/(θ_e)))
    
    # Term 2
    dI_2 = uconvert(u"kg*s^-2",(szpack_interp_T0(𝕡.vel, 𝕡.mu, nu)*u"MJy/sr")/τ)
...
function profile_grid_ksz(𝕡::AbstractGNFW{T}; N_z=256, N_logM=256, N_logθ=512, z_min=1e-3, z_max=5.0, 
              logM_min=11, logM_max=15.7, logθ_min=-16.5, logθ_max=2.5) where T #logM_max=15.7

    logθs = LinRange(logθ_min, logθ_max, N_logθ)
    redshifts = LinRange(z_min, z_max, N_z)
    logMs = LinRange(logM_min, logM_max, N_logM)

    return profile_grid_ksz(𝕡, logθs, redshifts, logMs)
end


function profile_grid_ksz(𝕡::AbstractGNFW{T}, logθs, redshifts, logMs) where T
...
function build_interpolator_ksz(model::AbstractGNFW; cache_file::String="",
    ...
    if overwrite || (isfile(cache_file) == false)
        ...
        prof_logθs, prof_redshift, prof_logMs, prof_y = profile_grid_ksz(model; 
...
function paint_ksz!(m, p::XGPaint.AbstractProfile, psa, sitp, 
    ...
    for i in irange
        ...
        profile_paint_ksz!(m, p, α₀, δ₀, psa, sitp, z, mh, θmax_)
    end
end
...
function paint_ksz!(m, p::XGPaint.AbstractProfile, psa, sitp, masses::AV, 
    ...
    Threads.@threads for i in 1:Threads.nthreads()
        ...
        paint_ksz!(m, p, psa, sitp, masses, redshifts, αs, δs, i1:i2)
    end
