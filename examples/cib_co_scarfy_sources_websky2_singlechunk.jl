...
@time halo_vrad = h5open("/home/dongwooc/scratchspace/websky2_ubermerge_halos-extra.h5","r") do file
   return read(file,"vpec_kms")
end
...
modelCIB = CIB_Scarfy{Float32}(nside=4096)
model_CO = CO_CROWNED{Float32}(nside=4096)
...
function write_chunk(
    ...
    @time sourcesCIB = generate_sources(modelCIB, cosmo, pos, mass, vrad);
    @time sources_CO = process_sources(model_CO, sourcesCIB, modelCIB);
    ...
    if (chunk_index > 1) && isfile(filename)
...
##
function run_all_chunks(output_dir, halo_pos, halo_mass, halo_vrad, freqs; N_chunks=8)
    ...
    write_chunk(output_dir, chunk_index, modelCIB, model_CO, cosmo,
        pos[:,validmask], mass[validmask], vrad[validmask], freqs)
    # save maps
...
run_all_chunks(scratch_dir, halo_pos, halo_mass, halo_vrad, freqs)
